...
- **Backend:** **debootstrap** is used to assemble the Debian-based live system.
...
> [!NOTE]
> The `build.sh` script is the entry point that orchestrates `debootstrap` and `mksquashfs`. It is generated by the CI workflow, but you can find its logic in `.github/workflows/kiba.yml`.
