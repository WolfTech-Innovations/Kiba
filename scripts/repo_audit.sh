...
if [ -f "build.sh" ]; then
    ...
    # Verify ldconfig after PaperDE build (only if paperde-src is present)
...
# chmod 777
if grep -rE "chmod (0?777|777)" . --exclude-dir={.git,.github,node_modules} --exclude="repo_audit.sh" --exclude="audit-*.yml"; then
