...
if [ -f "build.sh" ]; then
    ...
    # Verify ldconfig after PaperDE build (if PaperDE is being built)
...
# chmod 777
if grep -rE "chmod (0?777|777)" . --exclude-dir=.git --exclude-dir=.Jules --exclude="*.md" --exclude="repo_audit.sh"; then
    log_error "Found dangerous chmod 777"
fi
# Plaintext chpasswd
if grep -r "chpasswd" . --exclude-dir=.git --exclude-dir=.Jules --exclude="*.md" --exclude="repo_audit.sh" --exclude="workflows_to_add.txt" | grep -v "chpasswd -e"; then
    log_error "Found potential plaintext password leak via chpasswd (use -e for hashed passwords)"
fi
