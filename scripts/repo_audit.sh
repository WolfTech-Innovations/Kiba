...
if [ -f "build.sh" ]; then
    ...
    if grep -q "paperde" build.sh; then
...
# chmod 777
if grep -rE "chmod (0?777|777)" . --exclude-dir=.git --exclude=repo_audit.sh --exclude-dir=.Jules; then
    log_error "Found dangerous chmod 777"
fi
# chpasswd usage
if grep -r "chpasswd" . --exclude-dir=.git --exclude=repo_audit.sh --exclude-dir=.Jules --exclude="*.md" --exclude="workflows_to_add.txt" | grep -v "liveuser:live"; then
    log_error "Potential hardcoded password in chpasswd usage"
fi
