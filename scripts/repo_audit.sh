...
if [ -f "build.sh" ]; then
    ...
    # Verify ldconfig after PaperDE build (only if PaperDE source build is present)
...
# chmod 777 (exclude audit scripts/workflows to avoid false positives)
if grep -rE "chmod 0?777" . --exclude-dir=.git --exclude="repo_audit.sh" --exclude="audit-*.yml"; then
