...
# chmod 777
# Exclude .git, .Jules, and known safe files or the audit script itself
if grep -rE "chmod (0?777|777)" . \
    --exclude-dir=.git --exclude-dir=.Jules \
    --exclude="scripts/repo_audit.sh" --exclude="*.md" \
    --exclude="workflows_to_add.txt" | grep -v "scripts/repo_audit.sh" | grep -q .; then
...
if grep -r "chpasswd" . \
    --exclude-dir=.git --exclude-dir=.Jules \
    --exclude="scripts/repo_audit.sh" --exclude="*.md" \
    --exclude="workflows_to_add.txt" | grep -v "scripts/repo_audit.sh" | grep -v "chpasswd -e" | grep -q .; then
