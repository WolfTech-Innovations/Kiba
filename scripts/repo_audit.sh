...
# chpasswd security check (should use -e for encrypted passwords)
if grep -r "chpasswd" . --exclude-dir=.git --exclude="repo_audit.sh" --exclude-dir=.github --exclude="*.md" --exclude="workflows_to_add.txt" | grep -v "chpasswd -e"; then
