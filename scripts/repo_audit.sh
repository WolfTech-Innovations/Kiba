...
# Insecure chpasswd
if grep -r "chpasswd" . --exclude-dir=.git --exclude="*.md" --exclude="workflows_to_add.txt" | grep -vE "chpasswd -e|repo_audit.sh"; then
    log_error "Found insecure chpasswd usage (missing -e for encrypted passwords)"
fi
# Insecure makepkg
if grep -r "makepkg" . --exclude-dir=.git --exclude="*.md" --exclude="workflows_to_add.txt" | grep -v "repo_audit.sh" | grep -q "\-\-skippgpcheck"; then
    log_error "Found insecure makepkg usage (--skippgpcheck)"
fi
