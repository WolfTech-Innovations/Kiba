...
# chmod 777
# Exclude current script, journals, and markdown documentation to prevent false positives
if grep -rE "chmod (0?777|777)" . --exclude-dir={.git,.Jules} --exclude="repo_audit.sh" --exclude="*.md"; then
    log_error "Found dangerous chmod 777"
fi
# chpasswd without -e (encrypted flag) indicates plaintext passwords in scripts
# Exclude workflows_to_add.txt as it contains references to audit names
if grep -r "chpasswd" . --exclude-dir={.git,.Jules} --exclude={"repo_audit.sh","workflows_to_add.txt"} --exclude="*.md" | grep -v "\-e"; then
    log_error "Found chpasswd usage without -e flag (plaintext password risk)"
fi
