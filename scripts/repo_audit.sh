...
if [ -f "build.sh" ]; then
    ...
    # Verify ldconfig after PaperDE build (if PaperDE build is present)
...
# chmod 777
if grep -rE "chmod (0?777|777)" . --exclude-dir=.git --exclude-dir=.github --exclude-dir=.Jules --exclude="*.md" --exclude="workflows_to_add.txt" --exclude="repo_audit.sh"; then
    log_error "Found dangerous chmod 777"
fi
# chpasswd without -e (plaintext password risk)
if grep -r "chpasswd" . --exclude-dir=.git --exclude-dir=.github --exclude-dir=.Jules --exclude="*.md" --exclude="workflows_to_add.txt" --exclude="repo_audit.sh" | grep -v "chpasswd -e"; then
