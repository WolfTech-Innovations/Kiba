...
# Hardcoded passwords in chpasswd
if grep -r "chpasswd" . --exclude-dir=.git --exclude="repo_audit.sh" --exclude="*.md" | grep -vE "chpasswd\s*<"; then
    if grep -rE "echo\s+['\"].+:.+['\"]\s*\|\s*chpasswd" . --exclude-dir=.git --exclude="repo_audit.sh" --exclude="*.md"; then
        log_error "Found potential hardcoded password in chpasswd"
    fi
fi
