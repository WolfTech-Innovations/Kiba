...
if [ -f "build.sh" ]; then
    ...
    if ! grep -q "linux-cachyos" build.sh; then
        log_error "build.sh is missing linux-cachyos kernel"
    fi
...
# chpasswd usage check
if grep -r "chpasswd" . --exclude-dir=.git --exclude="repo_audit.sh" --exclude="*.md" --exclude="workflows_to_add.txt" | grep -v "\-e"; then
    log_error "Found insecure chpasswd usage (missing -e flag for hashed passwords)"
fi
