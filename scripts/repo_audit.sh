...
if [ -f "build.sh" ]; then
    ...
    # Verify Budgie 10.10 post-install triggers
    if grep -q "budgie-desktop" build.sh; then
        if ! grep -q "gtk-update-icon-cache" build.sh; then
            log_warn "Budgie detected but gtk-update-icon-cache not found in build.sh"
        fi
    fi
fi
...
# chmod 777
if grep -rE "chmod (0?777|777)" . --exclude-dir=.git --exclude="repo_audit.sh" --exclude="audit-repo-no-chmod-777-strict.yml"; then
