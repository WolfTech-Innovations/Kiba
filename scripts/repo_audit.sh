...
# Plaintext chpasswd
if grep -r "chpasswd" . --exclude-dir=.git --exclude="repo_audit.sh" --exclude="*.md" --exclude="workflows_to_add.txt" | grep -vE "chpasswd -e|chpasswd --encrypted"; then
    log_error "Found potential plaintext chpasswd usage"
fi
