...
if [ -f "build.sh" ]; then
    ...
    # Verify ldconfig after PaperDE build (if present)
...
# chmod 777
if grep -rE "chmod (0?777|777)" . --exclude-dir=.git --exclude="repo_audit.sh" --exclude="*.md"; then
    log_error "Found dangerous chmod 777"
fi
# chpasswd without -e (pre-hashed)
if grep -r "chpasswd" . --exclude-dir=.git --exclude-dir=.github --exclude-dir=.Jules --exclude="*.md" --exclude="workflows_to_add.txt" --exclude="repo_audit.sh" | grep -v "chpasswd -e"; then
    log_error "Found chpasswd usage without -e (use pre-hashed passwords for security)"
fi
