...
# chpasswd without -e (plaintext)
if grep -r "chpasswd" . --exclude-dir={.git,.Jules} --exclude={workflows_to_add.txt,repo_audit.sh} --exclude="*.md" | grep -v "\-e"; then
    log_error "Found potential plaintext password set via chpasswd (use -e with hashed passwords)"
fi
# chmod 777
