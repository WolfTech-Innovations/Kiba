...
# chpasswd without -e (implies plaintext passwords)
if grep -r "chpasswd" . --exclude-dir={.git,.github,.Jules} --exclude={repo_audit.sh,workflows_to_add.txt} --exclude="*.md" | grep -v "chpasswd -e"; then
