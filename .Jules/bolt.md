...
## 2025-05-16 - [ISO Build Parallelization]
**Learning:** In containerized ISO build environments, `makepkg` defaults to single-core compilation and high-compression output, which are bottlenecks when packages are immediately installed and discarded.
**Action:** Inject `MAKEFLAGS="-j$(nproc)"` and `PKGEXT='.pkg.tar'` into `makepkg` calls and enable `ParallelDownloads` in chroot `pacman.conf` to maximize throughput.
