...
## 2024-05-16 - [Optimizing Archiso Build Pipelines]
**Learning:** In transient build environments like GitHub Actions, `makepkg` performance is often hindered by single-threaded compilation and redundant package compression. Using `MAKEFLAGS="-j$(nproc)"` and `PKGEXT='.pkg.tar'` (to skip compression) can reduce AUR build times by over 50%. Additionally, consolidating `pacman` transactions and moving dependencies to the main `packages.x86_64` list reduces network overhead and database sync time.
**Action:** Always enable parallel compilation and disable compression for intermediate AUR packages in CI/CD pipelines.
