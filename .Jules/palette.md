...
## 2025-05-22 - [Accessibility: Informational Cards vs Interactive Elements]

**Learning:** When enhancing non-semantic layouts (e.g., card grids), `tabindex="0"` should only be applied to elements that trigger an action. Applying focus states to purely informational cards creates "tab traps" that frustrate keyboard users without providing functional value. Semantic ARIA roles (`role="list"`, `role="listitem"`) should be used instead to provide screen reader context without affecting tab order.
**Action:** Use ARIA roles for structural context on informational cards, but reserve `tabindex="0"` and `:focus-visible` styles exclusively for interactive buttons and links.
